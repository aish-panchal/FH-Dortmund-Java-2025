...
public class file_ops {
		...
		try (Stream<Path> files = Files.list(log_path)) {
			...
			files.filter(file -> file.getFileName().toString().endsWith(".txt"))
					.filter(file -> pattern.matcher(file.getFileName().toString()).find()).forEach(file -> {
						...
						logView.getItems().add(file.getFileName().toString());
					});
		} catch (IOException e) {
	...
	public static void openLog(String fileName, TextArea logContentArea) {
		...
		try {
			...
			System.out.println("Error reading file for opening: " + e.getMessage());
		}
	}
	...
	public static void simulateDataXchange(String fileName) {
		...
		try {
			...
			try (FileInputStream in_strm = new FileInputStream(log_path.toFile());
					FileOutputStream out_strm = new FileOutputStream("logs/byte_strm_output.txt")) {
				byte[] buffer = new byte[1024]; // arbitrary value of 1 KB chosen for simple read and write for
												// simulation reasons
				int bytesRead;
				...
				try (FileReader reader = new FileReader(log_path.toFile());
						FileWriter writer = new FileWriter("logs/char_strm_output.txt")) {
					int charactersRead;
