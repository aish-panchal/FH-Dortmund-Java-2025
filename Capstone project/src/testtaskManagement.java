...
class testtaskManagement {
	...
	void testavailable_vehicles() throws exception_handling.ZeroTonnesException, exception_handling.InvalidOrderException, exception_handling, exception_handling.VehicleNotFoundException {
	...
	void testop_vehicles() throws exception_handling.ZeroTonnesException, exception_handling.InvalidOrderException,
		...
		assertEquals(25, order.vehicles.size());//at the end vehicle shoould return to main avg list
	//	order.takeOrder(0,"toFactory");//invalid tone order
	...
	void testcallchargingStation() throws exception_handling.ZeroTonnesException,
			exception_handling.InvalidOrderException, exception_handling, exception_handling.VehicleNotFoundException {
		
		taskManagement order2 = new taskManagement(testdate,25);
		taskManagement order3 = new taskManagement(testdate,25);
		order2.takeOrder(10, "toDelivery");//5 vehicles end up in need of charging
		//order3.takeOrder(10, "toWarehouse");//5 vehicles end up in need of charging
		
		int avgToCharge=5;
		
		assertEquals(avgToCharge,order2.vehiclesInNeedOfCharging.size());// there should be 5 vehicles on the queue
		assertEquals(0, order3.vehiclesInNeedOfCharging.size());//there should be vehicles in the queue
	}
}
